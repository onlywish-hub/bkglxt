...
public class  AddArticleUI extends JDialog {
	...
	public static void main(String[] args) {
		try {
			AddArticleUI dialog = new AddArticleUI();
	...
	public AddArticleUI(ArticleManageUI ArticleUI) {
		showComponent();
	...
	public void showComponent() {
		...
		// 给窗体设置图标
		this.setIconImage(Toolkit.getDefaultToolkit().getImage(AddArticleUI.class.getResource("/top/picture/fox.png")));
	...
	// 获取用户输入到插入信息再到显示在面板上
	public void btnUpdateAction(ArticleManageUI ArticleUI) {
		// 获得所有的用户输入信息
...
//			txtAuthorID.setText("");
			...
			JOptionPane.showMessageDialog(null, "出错了", "提示", JOptionPane.ERROR_MESSAGE,icon);
		}
		...
		ArticleTableModel btm = new ArticleTableModel(articles);
		System.out.println(btm);
		// 4.2 把模型绑定在表格控件
		ArticleUI.Tables.setModel(btm);
	}
