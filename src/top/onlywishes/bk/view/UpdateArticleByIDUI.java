...
public class UpdateArticleByIDUI extends JDialog {
	...
	public static void main(String[] args) {
		try {
			UpdateArticleByIDUI dialog = new UpdateArticleByIDUI();
	...
	public UpdateArticleByIDUI() {
		showComponent();

	}


	public UpdateArticleByIDUI(ArticleManageUI ArticleUI, int article_id) {
	...
	public void init(int article_id) {
		ArticleDao bs = new ArticleDao();
		try {
			article = bs.getArticlesByArticleID(article_id);

			System.out.println(article+"kk");
			if(article != null) {
				// 将通过名字获取到的数据添加入图形界面中
...
//			构造新的表格模型
	...
	public void setLOGO() {
		...
		// 获取图片 三种图片格式都可以
//		java.awt.Image img = tk.getImage(UpdateArticleByIDUI.class.getResource("top/picture/fox.png")); ///前面少个/
		// 给窗体设置图标
		this.setIconImage(Toolkit.getDefaultToolkit().getImage(UpdateArticleByIDUI.class.getResource("/top/picture/fox.png")));
